...
if [ -n "$1" ];then
	if echo "$1" | grep -qF /;then
		...
		rom_script='$(call inherit-product, device/xiaomi/x6a/'$1'.mk)'
	fi
fi
	...
	for apps in vanilla gapps foss gapps-go;do
		for arch in arm64 arm a64;do
			for su in yes no;do
				...
				if [ "$apps" == "gapps" ];then
					apps_suffix="g"
					apps_script='$(call inherit-product, device/xiaomi/x6a/gapps.mk)'
					apps_name="with GApps"
				fi
				if [ "$apps" == "gapps-go" ];then
					apps_suffix="o"
					apps_script='$(call inherit-product, device/xiaomi/x6a/gapps-go.mk)'
					apps_name="Go"
				fi
				if [ "$apps" == "foss" ];then
					apps_suffix="f"
				...
				cat > ${target}.mk << EOF
\$(call inherit-product, device/xiaomi/x6a/base-pre.mk)
